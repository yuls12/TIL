...
class Attention(nn.Module):
    ...
    def forward(self, Q, K, V, mask=None, dk=64):
        ...
        # |w| = (batch_size, m, n)
        # 배치내의 각 샘플별 decoder의 각 time-step별 encoder의 각 time-step에 대한 attention weight값
        if mask is not None:
            assert w.size() == mask.size()
            w.masked_fill_(mask, -float('inf'))  # <Pad> 보완

        w = self.softmax(w / (dk**.5))  # normalize weights
        c = torch.bmm(w, V)
...
class MultiHead(nn.Module):
    ...
    def forward(self, Q, K, V, mask=None):
        ...
        QWs = self.Q_linear(Q).split(self.hidden_size //
                                     self.n_splits, dim=-1)  # parallel size list
        ...
        c = self.attn(
            # |QWs| = (batch_size * n_splits, m, hidden_size / n_splits)
            QWs, KWs, VWs,
            mask=mask,  # |mask| = (batch_size * n_splits, m, n)
            dk=self.hidden_size // self.n_splits,  # parallel size
        )
...
class MySequential(nn.Sequential):  # 입력과 출력을 다루기 위한 class
...
class Transformer(nn.Module):
    ...
    @torch.no_grad()
    def _generate_pos_enc(self, hidden_size, max_length):  # positional encoding
        ...
        enc[:, 0::2] = torch.sin(
            pos / 1e+4**dim.div(float(hidden_size)))  # 짝수 demension
        enc[:, 1::2] = torch.cos(
            pos / 1e+4**dim.div(float(hidden_size)))  # 홀수 demension

        return enc

    # position encoding을 필요한 부분만 가져와서 사용하는 부분
    ...
    def search(self, x, is_greedy=True, max_length=255):
        ...
        z = self.emb_dropout(self._position_encoding(self.emb_enc(x)))
        z, _ = self.encoder(z, mask_enc)
        # |z| = (batch_size, n, hidden_size) encoder의 output
        ...
        while is_decoding.sum() > 0 and len(indice) < max_length:
            ...
            # |h_t| = (batch_size, 1, hidden_size) hidden representation
            if prevs[0] is None:  # input 값
                prevs[0] = h_t
            else:
            ...
            y_hat_t = self.generator(h_t)  # log prob
